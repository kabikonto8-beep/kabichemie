...
(function () {
  ...
  if (impactCurve) {
    ...
    var updateImpactCurve = function () {
      ...
      curveStats.forEach(function (stat, i) {
        // każda liczba pojawia się, gdy linia "Zug" do jej miejsca
  ...
  document.querySelectorAll("[data-proc-arc]").forEach(function (arc) {
    ...
    core.setAttribute("title", "Klicken Sie, um Prozessphasen zu verstecken oder anzuzeigen");
    var hint = core.querySelector(".proc-hub__hint");
    if (!hint) {
      hint = document.createElement("span");
      hint.className = "proc-hub__hint";
      hint.textContent = "Zum Einklappen klicken";
    ...
    var toggle = function () {
      ...
      hint.textContent = hidden ? "Klicken Sie auf Expand" : "Zum Einklappen klicken";
      setReadyAfterDomino();
    };
  ...
  // --- animowany akordeon kart w sekcji "Was tun wir?" ---
  ...
  document.querySelectorAll("[data-gallery]").forEach(function (gallery) {
    ...
    if (dotsWrap) {
      ...
      items.forEach(function (_item, index) {
        ...
        dot.setAttribute("aria-label", "Gehen Sie zur Fallstudie" + (index + 1));
        dot.addEventListener("click", function () {
          scrollToIndex(index);
        });
  ...
  document.querySelectorAll(".contact-form").forEach(function (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (note) {
        note.hidden = false;
        note.textContent =
          "Vielen Dank! Das Demonstrationsformular akzeptierte Kontaktdaten. Um produktiv zu arbeiten, verbinden Sie es mit einer E-Mail-Box oder einem Formularsystem. In der Zwischenzeit schreiben Sie auf" +
  ...
  document.querySelectorAll("[data-savings-calculator]").forEach(function (form, calculatorIndex) {
    ...
    form.querySelectorAll(".calc2-input input[type='number']").forEach(function (input, inputIndex) {
      ...
      stepper.setAttribute("aria-label", "Wertänderung:" + fieldName);
      ...
      var makeButton = function (direction) {
        ...
        button.setAttribute("aria-label", (increase ? "Erhöhung" : "Abnahme") + "Feldwert" + fieldName);
        button.title = increase ? "Wertsteigerung" : "Wertminderung";
    ...
    var calcCond = function () {
      ...
      return {
        scale: scaleSav, salt: saltSav, total: scaleSav + saltSav,
        metrics: [
          { label: "Berechnete Sedimentdicke", value: thick, unit: "mm", decimals: 2 },
          { label: "Effizienzverlust", value: scaleLoss, kind: "percent" },
          { label: "Mniej odsolin rocznie", value: diff, unit: "t", decimals: 2 }
        ]
      };
    };
    ...
    var resultMessage = function (total) {
      return total > 250000
        ? "Potenzial zur Rückgewinnung hoher Kosten. Mal sehen, welche Aktionen die größte Wirkung auf die Pflanze haben können."
        : total > 80000
          ? "Klares Verbesserungspotenzial. Es lohnt sich zu bestimmen, welche Verluste zuerst reduziert werden können."
          : "Es lohnt sich, den Kesselstein und die Abschlämmung zu überprüfen, um die Möglichkeit der Kostensenkung zu ermitteln.";
    };
    ...
    var applyResultStructure = function (r, waiting) {
      ...
      if (out.msg) {
        out.msg.textContent = waiting
          ? "Wir analysieren die Parameter der Installation. Das Ergebnis erscheint nach Abschluss der Daten."
    ...
    var applyScrollResult = function (progress) {
      ...
      if (out.msg) {
        out.msg.textContent = resultProgress < 0.01
          ? "Potenzial wird mit den Installationsdaten erscheinen."
          : resultProgress < 0.985
            ? "Das Modell konvertiert die folgenden Installationsparameter."
  ...
  document.querySelectorAll("[data-coverflow]").forEach(function (cf) {
    ...
    if (dotsWrap) {
      cards.forEach(function (_c, i) {
        ...
        b.type = "button";
        b.setAttribute("aria-label", "Showindustrie" + (i + 1));
  ...
  // --- listy realizacji / bazy wiedzy: wyszukiwarka + paginacja ---
  ...
  Array.prototype.slice.call(document.querySelectorAll(".company-stories__list")).forEach(function (list) {
    ...
    if (!section) return;
    var input = section.querySelector("[data-stories-search]");
    var pager = section.querySelector("[data-stories-pager]");
    var empty = section.querySelector("[data-stories-empty]");
    ...
    function matches() {
      ...
      return items.filter(function (a) {
        var strong = a.querySelector("strong");   // tytuł
        var cat = a.querySelector("span");          // kategoria (etykieta)
        var hay = ((cat ? cat.textContent + " " : "") + (strong ? strong.textContent : a.textContent)).toLowerCase();
    ...
    function pbtn(label, target, disabled, active, aria) {
      return '<button type="button" class="stories-pager__btn' + (active ? " is-active" : "") + '"'
        + (disabled ? " disabled" : "")
        + ' data-page="' + target + '" aria-label="' + aria + '"'
    ...
    function drawPager(total) {
      ...
      pager.hidden = false;
      var html = pbtn("‹", page - 1, page <= 1, false, "Vorherige Seite");
      pageList(page, total).forEach(function (n) {
        if (n === "...") { html += '<span class="stories-pager__gap" aria-hidden="true">…</span>'; return; }
        html += pbtn(String(n), n, false, n === page, "Seite" + n);
      });
      html += pbtn("›", page + 1, page >= total, false, "Nächste Seite");
    ...
    if (input) {
      ...
      input.addEventListener("keydown", function (e) {
        if (e.key === "Escape") { input.value = ""; page = 1; render(); }
      });
    }
    if (pager) {
