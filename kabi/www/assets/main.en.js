...
(function () {
  ...
  if (impactCurve) {
    ...
    var updateImpactCurve = function () {
      ...
      curveStats.forEach(function (stat, i) {
        // każda liczba pojawia się, gdy linia "draw" do jej miejsca
  ...
  document.querySelectorAll("[data-proc-arc]").forEach(function (arc) {
    ...
    core.setAttribute("title", "Click to hide or show process stages");
    ...
    var toggle = function () {
      ...
      hint.textContent = hidden ? "Click to expand" : "Click to collapse";
      setReadyAfterDomino();
    };
  ...
  // --- animowany akordeon kart w sekcji "What do we do?" ---
  ...
  document.querySelectorAll(".contact-form").forEach(function (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (note) {
        note.hidden = false;
        note.textContent =
          "Thank you! The demonstration form accepted contact details. To work productively, connect it to an email box or form system. In the meantime, write on" +
  ...
  document.querySelectorAll("[data-savings-calculator]").forEach(function (form, calculatorIndex) {
    ...
    form.querySelectorAll(".calc2-input input[type='number']").forEach(function (input, inputIndex) {
      ...
      stepper.setAttribute("aria-label", "Change of value:" + fieldName);
      ...
      var makeButton = function (direction) {
        ...
        button.setAttribute("aria-label", (increase ? "Increase" : "Decrease") + "field value" + fieldName);
        button.title = increase ? "Increase Value" : "Decrease Value";
    ...
    var calcCond = function () {
      ...
      return {
        scale: scaleSav, salt: saltSav, total: scaleSav + saltSav,
        metrics: [
          { label: "Calculated sediment thickness", value: thick, unit: "mm", decimals: 2 },
          { label: "Loss of efficiency", value: scaleLoss, kind: "percent" },
          { label: "Mniej odsolin rocznie", value: diff, unit: "t", decimals: 2 }
        ]
      };
    };
    ...
    var resultMessage = function (total) {
      ...
      return pln > 250000
        ? "High cost recovery potential. Let's see which actions can bring the biggest effect to the plant."
        : pln > 80000
          ? "Clear potential for improvement. It is worth determining which losses can be reduced first."
          : "It is worth checking the scale and blowdown to determine the possibility of reducing costs.";
    };
    ...
    var applyResultStructure = function (r, waiting) {
      ...
      if (out.msg) {
        out.msg.textContent = waiting
          ? "We analyze the parameters of the installation. The result will appear after completion of the data."
    ...
    var applyScrollResult = function (progress) {
      ...
      if (out.msg) {
        out.msg.textContent = resultProgress < 0.01
          ? "Potential will appear with installation data."
          : resultProgress < 0.985
            ? "The model converts the following installation parameters."
  ...
  document.querySelectorAll("[data-coverflow]").forEach(function (cf) {
    ...
    if (dotsWrap) {
      cards.forEach(function (_c, i) {
        ...
        b.type = "button";
        b.setAttribute("aria-label", "Show Industry" + (i + 1));
  ...
  Array.prototype.slice.call(document.querySelectorAll(".company-stories__list")).forEach(function (list) {
    ...
    function drawPager(total) {
      ...
      pager.hidden = false;
      var html = pbtn("‹", page - 1, page <= 1, false, "Previous page");
      pageList(page, total).forEach(function (n) {
        if (n === "...") { html += '<span class="stories-pager__gap" aria-hidden="true">…</span>'; return; }
        html += pbtn(String(n), n, false, n === page, "Page" + n);
      });
      html += pbtn("›", page + 1, page >= total, false, "Next page");
